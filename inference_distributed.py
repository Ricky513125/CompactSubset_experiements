...
# 尝试导入 japanese_text_normalizer，如果不存在则定义占位函数
try:
    from japanese_text_normalizer import normalize_japanese_text
except ImportError:
    print("警告: japanese_text_normalizer 模块未找到，使用占位函数")
...
def process_distributed(
    ...
    for item_info in iterator:
        ...
        # 对于DMSC/MovieReview数据集，如果context为空，使用continuation_prefix构建context
        if not context and dataset_name in ['DMSC', 'MovieReview']:
            continuation_prefix = data_item.get('continuation_prefix', '')
            if continuation_prefix:
                # 将continuation_prefix转换为context格式
                # 格式: [{"role": "user", "content": continuation_prefix}]
                context = [{"role": "user", "content": continuation_prefix}]
            else:
                # 如果既没有context也没有continuation_prefix，跳过
                error_count += 1
                continue
        elif not context:
            # 其他数据集必须有context
        ...
        while generated_count < num_samples and total_attempts < max_total_attempts:
            ...
            if generated_count == 0:
                # 第一条用更合理的参数，让模型能生成正常文本
                temperature = 0.7
            elif retry_count < 5:
                # 前5次重试，温度缓慢增加
                temperature = 0.7 + retry_count * 0.05
            else:
                # 5次之后，温度增加更快，但限制在0.9以内
                temperature = 0.85 + (retry_count - 5) * 0.01
            ...
            try:
                result = generate_continuations(
                    ...
                    messages=messages,
                    num_samples=1,
                    max_new_tokens=min(128, max_new_tokens),  # ✅ 增加到128，给模型足够空间生成完整句子
                    device=model_device,
                    do_sample=True,
                    temperature=temperature,
                    top_p=0.9 if temperature < 0.8 else 0.95,  # ✅ 放宽top_p，增加生成多样性
                    top_k=50 if temperature < 0.8 else 100,  # ✅ 放宽top_k，增加生成空间
                    repetition_penalty=1.2,  # ✅ 降低到1.2，避免过度抑制导致只生成标点符号
                    no_repeat_ngram_size=4,  # ✅ 增加到4，允许正常的短语重复（如"你好"、"谢谢"）
                ...
                try:
                    result = generate_continuations(
                        ...
                        messages=messages,
                        num_samples=1,
                        max_new_tokens=min(128, max_new_tokens),  # ✅ 增加到128
                        device=model_device,
                        do_sample=False,
                        repetition_penalty=1.2,  # ✅ 降低到1.2
                        no_repeat_ngram_size=4,  # ✅ 增加到4
                        max_output_length=max_output_length,
                        is_japanese_task=is_japanese_task,
                        suppress_emoji=True,  # ✅ 启用 emoji 抑制
