...
from prompt_builder_LovinkDialogue import build_prompt  # ✅ 使用 Lovink 版本（包含详细模板）
from transformers import AutoTokenizer, AutoModelForCausalLM
# 导入增强版清理函数
from inference_enhanced_clean import enhanced_clean_model_output
try:
...
def get_user_info_from_leaderboard(sample: dict, train_data: list) -> dict:
    ...
    ✅ 修复：正确合并 user.profile 和 user.personality.structured
    """
    # 从测试样本中获取user_hash
    ...
    # ✅ 修复：使用与训练时相同的逻辑获取完整的 user_profile
    user = sample.get('user', {})
    user_profile = _get_full_user_profile(user)
    ...
    # 如果测试样本中没有完整profile，从训练数据中获取
    if (not user_profile or not user_profile.get('dimensions')) and user_train_samples:
        first_sample = user_train_samples[0]
        train_user = first_sample.get('user', {})
        user_profile = _get_full_user_profile(train_user)
...
def _get_full_user_profile(user: dict) -> dict:
    """
    获取完整的用户 profile（合并 user.profile 和 user.personality.structured）
    与 data_loader.py 中的 get_user_profile 逻辑一致
    """
    ...
    # ✅ 关键：将 personality.structured 作为 dimensions 字段
    if structured:
        full_profile['dimensions'] = structured
    
    # 添加 unstructured 文本分析（如果有）
...
def generate_continuations(
    ...
    messages,
    num_samples=5,
    max_new_tokens=4096,  # 增加到512，给模型足够空间写完"废话"，后续通过清洗函数截断
    device=None,
    do_sample=True,
    temperature=0.7,
    top_p=0.9,
    top_k=50,  # 添加top_k参数
    num_beams=None,  # beam search的beam数量
    repetition_penalty=1.2,  # 增加重复惩罚
    no_repeat_ngram_size=4,  # 增加n-gram大小
    max_output_length=4096,  # 输出文本最大字符数
    is_japanese_task=False,  # 是否为日语任务
    suppress_emoji=True,  # ✅ 新增：是否抑制 emoji 生成
    ...
    if is_japanese_task:
        # 不添加额外引导词，让模型自然生成（过度引导可能导致生成质量下降）
        # 模型在训练时已经学会了直接生成日语，不需要额外引导
        pass  # text保持不变
    ...
    inputs = tokenizer(
        ...
        return_tensors="pt",
        truncation=True,
        max_length=16384  # ✅ 从4096增加到16384，确保prompt不被截断
    ).to(device)
    ...
    if num_beams is not None and num_beams > 1:
        ...
        generation_kwargs["num_return_sequences"] = min(num_samples, num_beams)
        generation_kwargs["do_sample"] = False  # beam search不使用采样
        ...
        generation_kwargs["top_k"] = top_k  # 使用参数传入的top_k值
    else:
        # Greedy decoding
        generation_kwargs["do_sample"] = False
        generation_kwargs["num_return_sequences"] = 1
    
    # === 构建 LogitsProcessorList ===
    ...
    if hasattr(model.config, 'vocab_size') and tokenizer.vocab_size < model.config.vocab_size:
        ...
        processors.append(VocabSizeLimiter(tokenizer.vocab_size))
    ...
    processors.append(LogitsCleaner())
    ...
    processors.append(RepetitionPenaltyProcessor(penalty=1.1))
    
    # # 4. ✅ Emoji 抑制处理器（新增）
    # if suppress_emoji and emoji_suppression_mode != "off":
    #     try:
    #         from emoji_logits_processor import create_emoji_suppression_processor
            
    #         emoji_processor = create_emoji_suppression_processor(
    #             tokenizer=tokenizer,
    #             mode=emoji_suppression_mode,
    #             bias_value=emoji_bias_value
    #         )
            
    #         if emoji_processor is not None:
    #             processors.append(emoji_processor)
    #     except ImportError as e:
    #         print(f"⚠️  警告: 无法导入 emoji_logits_processor: {e}")
    #         print(f"   Emoji 抑制功能将被跳过")
    ...
    try:
        ...
        # ✅ 关键修复：在解码前移除 emoji token IDs（避免生成乱码 �）
        # if suppress_emoji:
        #     try:
        #         # 获取 emoji token IDs
        #         from emoji_filter import get_emoji_token_ids
        #         emoji_token_ids = get_emoji_token_ids(tokenizer)
        #         emoji_token_set = set(emoji_token_ids)
                
        #         # 过滤掉所有 emoji tokens
        #         non_emoji_mask = torch.tensor([tid.item() not in emoji_token_set for tid in cleaned_gen])
        #         if non_emoji_mask.any():
        #             cleaned_gen = cleaned_gen[non_emoji_mask]
        ...
        # 解码（使用 errors='ignore' 忽略无法解码的字符）
        try:
            decoded_text = tokenizer.decode(cleaned_gen, skip_special_tokens=True, errors='ignore').strip()
            ...
            # --- 关键：先规范化（转换中英文），再清洗 ---
            # 这样可以避免清洗函数误删日语数字（如"四"）
            if is_japanese_task:
                from japanese_text_normalizer import normalize_japanese_text
                decoded_text = normalize_japanese_text(decoded_text)
            
            # --- 调用增强版清洗函数 ---
            # 注意：日语任务中不要再移除英文（已经在规范化中处理）
            # remove_emoji 参数默认为 True，不需要显式传递
            # cleaned_text = enhanced_clean_model_output(
            #     decoded_text, 
            #     max_length=max_output_length,
            #     is_japanese_task=is_japanese_task,
            #     remove_english=False,  # 改为False，避免误删已规范化的内容
            #     remove_repeats=True
            #     # remove_emoji=True  # 默认值，不需要显式传递
            # )
...
def process_scenario(
    ...
    max_new_tokens: int = 4096,  # 默认增加到512
    ...
    base_model_path = "/mnt/parallel/models/Qwen3-8B"
    # 加载测试数据
    ...
    if not is_lora_model:
        ...
        try:
            if torch.cuda.is_available():
                # 使用 device_map 让 transformers 自动处理设备分配
    ...
    for sample_idx, sample in enumerate(tqdm(test_data, desc="生成进度")):
        ...
        for collection in collections:
            ...
            for data_item in data_items:
                ...
                max_retries_per_sample = 10  # 每个样本最多重试10次
                generated_count = 0
                total_attempts = 0
                max_total_attempts = num_samples * max_retries_per_sample  # 总尝试次数上限
                
                while generated_count < num_samples and total_attempts < max_total_attempts:
                    total_attempts += 1
                    
                    # 动态调整temperature，如果重试多次，增加temperature以增加多样性
                    retry_count = total_attempts - generated_count
                    temperature = 0.7 + min(retry_count * 0.05, 0.3)  # 最高1.0
                    ...
                    try:
                        ...
                        if result and len(result) > 0:
                            ...
                            # 检查是否重复（使用标准化后的文本比较）
                            normalized_new = new_continuation.strip().lower()
                            is_duplicate = False
                            ...
                            for existing in continuations:
                                ...
                                if normalized_new in normalized_existing or normalized_existing in normalized_new:
                                    # 如果长度差异不大，认为是重复
                                    if abs(len(normalized_new) - len(normalized_existing)) < 10:
                                        is_duplicate = True
                                        break
                            
                            if is_duplicate:
                                if retry_count <= 3:  # 只在前几次重试时打印
                                    print(f"  检测到重复 ({generated_count+1}/{num_samples})，重试中... (temperature={temperature:.2f})")
                            else:
                                # 不重复，添加到结果
                                continuations.append(new_continuation)
                                generated_count += 1
                                if generated_count < num_samples:
                                    print(f"  ✓ 已生成 {generated_count}/{num_samples} 个不重复的continuations")
                        ...
                        if "CUDA" in str(e) or "device-side assert" in str(e) or "inf" in str(e) or "nan" in str(e):
                            ...
                            try:
                                ...
                                if result and len(result) > 0 and result[0] not in continuations:
                                    continuations.append(result[0])
                                    generated_count += 1
                            except Exception as e2:
                                print(f"  警告: Greedy生成失败: {e2}")
                                break
                        else:
                            raise
                    except Exception as e:
                ...
                # 如果没有生成足够的样本，打印警告
                if generated_count < num_samples:
                    print(f"  警告: 只生成了 {generated_count}/{num_samples} 个不重复的continuations（尝试了{total_attempts}次）")
