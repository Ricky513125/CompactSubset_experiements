...
任务名称: LovinkDialogue (profile_and_context)
开始时间: $timestamp
Job ID: $JOB_ID
主机名: $(hostname)
工作目录: $WORK_DIR
...
torchrun --nproc_per_node=8 --master_port=29500 \\
    ...
    --output_dir outputs/LovinkDialogue_profile_and_context_0221_1 \\
    --max_epochs 50 \\
    --early_stopping_patience 3 \\
    --early_stopping_threshold 0.001 \\
    --val_ratio 0.1 \\
    --wandb_project Qwen3-LovinkDialogue \\
    --wandb_run_name profile_and_context_0221_1 \\
    --prompt_style simple
"
...
# 3. 运行训练
echo
run_training "LovinkDialogue" 29500 \
    train_distributed_LovinkDialogue.py \
    --config config_LovinkDialogue.json \
    --deepspeed ds_config_zero2.json \
    --ablation_config profile_and_context \
